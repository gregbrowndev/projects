...
trait CoreApplication[F[_]: Sync] {
  ...
  def updateJobInfo(cmd: Command.UpdateJobInfo): F[Either[DomainError.JobNotFound, Unit]]
  def deleteJob(cmd: Command.DeleteJob): F[Either[DomainError.JobNotFound, Unit]]
