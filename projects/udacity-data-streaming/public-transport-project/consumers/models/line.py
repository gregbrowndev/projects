...
class Line:
    ...
    def process_message(self, message):
        ...
        topic = message.topic()
        if topic == "org.chicago.cta.connect.stations.table.v1": # Set the conditional correctly to the stations Faust Table
        ...
        elif "org.chicago.cta.station.arrival." in topic: # Set the conditional to the arrival topic
            logger.info("processing arrival message")
            self._handle_arrival(message)
        elif topic == "TURNSTILE_SUMMARY": # Set the conditional to the KSQL Turnstile Summary Topic
            logger.info("processing turnstile message")
